	...
	public function LoginPage(Request $req)
{
    ...
    if ($req->reqMethod == "POST") {
        ...
        if (User::login($email, $password) === true) {
            ...
            $error = "Téves Email-cím és/vagy jelszó!";
        }
    }
	...
	public function SignUpPage(Request $req)
	{
		...
		if ($req->reqMethod == "POST") {
			...
			// check if there was an error
			if ($result == "EMAIL_ALREADY_IN_USE") {
				$error2 = "Az email már létezik"; // set error message
			} elseif ($result == "USER_CREATED") {
				$cart = Cart::getCart();
				...
				$twig = $this->setTwigEnvironment();
				$welcomecontent = $twig->render('welcome.html.twig', ['cart' => $data, 'allproduct' => $maindata, 'vasarlo' => $name, 'varosok' => $varosok, 'error2' => $error2]);
				$mail->CharSet = "UTF-8";
				$mail->isSMTP();
				...
				$error = "Sikeres regisztráció, jelentkezzen be.";
			}
		}

		$twig = $this->setTwigEnvironment();
		$html = $twig->render('pages/auth/register.html.twig', ['varosok' => $varosok, 'error' => $error, 'error2' => $error2]);
		echo $html;
	}
}
