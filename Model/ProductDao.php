...
class ProductDao implements ICrudDao
{
    ...
    public static function allapotokall()
    {
        $dbObj = new Database();
        $conn = $dbObj->getConnection();
    ...
    public static function markakall()
    {
        $dbObj = new Database();
        $conn = $dbObj->getConnection();
    ...
    public static function userOrders()
    {
        ...
        if (!empty($user->id)) {
            ...
            AS osszar,
              GROUP_CONCAT(DISTINCT cipo.megnevezes SEPARATOR '\n') AS termekek FROM `vasarlasok` INNER JOIN `vasarlo` ON `vasarlasok`.`vasarlok_id` = `vasarlo`.`id` 
INNER JOIN `fizetesimodok` ON `vasarlasok`.`fizetesimod_id` = `fizetesimodok`.`id` INNER JOIN vasarlas_tetelek on vasarlasok_id = vasarlasok.id INNER JOIN cipo on cipo.id = vasarlas_tetelek.cipo_id
 WHERE vasarlasok.vasarlok_id = ? GROUP BY vasarlas_tetelek.vasarlasok_id  ";
...
GROUP_CONCAT(DISTINCT cipo.megnevezes SEPARATOR '\n') AS termekek FROM `vasarlasok` INNER JOIN `vasarlo` ON `vasarlasok`.`vasarlok_id` = `vasarlo`.`id` INNER JOIN `fizetesimodok` ON `vasarlasok`.`fizetesimod_id` = `fizetesimodok`.`id` INNER JOIN vasarlas_tetelek on vasarlasok_id = vasarlasok.id INNER JOIN cipo on cipo.id = vasarlas_tetelek.cipo_id WHERE vasarlasok.vasarlok_id = 0 GROUP BY vasarlas_tetelek.vasarlasok_id ";
    ...
    public static function update()
    {
        ...
        $sql = "SELECT fnev, fnev2, fnev3, fnev4, fnev5, leiras FROM cipo WHERE id=:id";
    ...
    public static function resize($kep, $target_name)
    {
    ...
    public static function adminorderlist()
    {
        ...
        $sql = "SELECT `vasarlasok`.id, `vasarlo`.nev, `vasarlo`.email, 
`vasarlasok`.mikor, `vasarlo`.cim, `fizetesimodok`.megnevezes, `vasarlasok`.megjegyzes, `vasarlasok`.cim, `vasarlasok`.allapot, 
 GROUP_CONCAT(DISTINCT cipo.megnevezes SEPARATOR '\n') AS termekek FROM `vasarlasok`
 INNER JOIN `vasarlo` ON `vasarlasok`.`vasarlok_id` = `vasarlo`.`id` 
INNER JOIN `fizetesimodok` ON `vasarlasok`.`fizetesimod_id` = `fizetesimodok`.`id`
 INNER JOIN `vasarlas_tetelek` ON `vasarlasok`.`id` = `vasarlas_tetelek`.`vasarlasok_id`
 INNER JOIN `cipo` ON `cipo`.`id` = `vasarlas_tetelek`.`cipo_id`

 GROUP BY vasarlasok.id  ORDER BY `vasarlasok`.id;";
        $statement = $conn->prepare($sql);
        $statement->setFetchMode(\PDO::FETCH_OBJ);
        $statement->execute();
        return $statement->fetchAll();
    ...
    public static function orderupdate()
{
    ...
    public static function havibevetel()
    {
        ...
        $sql = "SELECT  SUM(vasarlas_tetelek.ar * vasarlas_tetelek.mennyiseg) AS osszesen
            FROM `vasarlasok` 
            INNER JOIN `vasarlas_tetelek` ON `vasarlasok`.`id` = `vasarlas_tetelek`.`vasarlasok_id` 
